...
const ThumbnailGallery = ({ images }: { images: images }) => {
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row flex-1 flex-grow-0 justify-start items-start gap-5">
      <div className="">
        <div className="grid md:grid-cols-1 grid-cols-3 gap-4 justify-center items-center">
          {images.map((image, index) => (
            <div
              key={index}
              className={`cursor-pointer max-w-[110px] md:max-w-[130px] rounded-2xl ${
        ...
        {images.length > 0 && (
          <div>
            <Image
              ...
              className="h-full w-full border-2 border-gray-200 object-cover object-center shadow-sm dark:border-gray-800 sm:rounded-2xl"
            />
          </div>
        )}
      </div>
